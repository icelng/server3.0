...
/* 函数名: int rsa_check_keypair(char *pubkey, char *prikey)
 * 功能: 验证RSA秘钥对的有效性。。
 * 参数: char *pubkey, 公钥
 *       char *prikey, 密钥
 * 返回值: 1, 有效
 *        <0, 无效或者检查错误
 */
int rsa_check_keypair(char *pubkey, char *prikey){
    char check[16];
    char cipher[4096], plain[4096];
    strcpy(check, "check");
    if(rsa_pub_encrypt(pubkey, check, cipher) < 0){
        /*公钥加密失败*/
        return -1;
    }
    if(rsa_priv_decrypt(prikey, cipher, plain) < 0){
        /*密钥解密失败*/
        return -2;
    }
    if(strcmp(check, plain) != 0){
        /*加解密过程中，信息丢失*/
        return -3;
    }
    return 1;
...
/*-------------------------------------------------------
 通过公钥长度获取加密长度
 注意:函数参数pubKeyLen是经过Base64编码之后的秘钥
-------------------------------------------------------*/
...
/*-------------------------------------------------------
 ...
 注意:函数参数priKeyLen是经过Base64编码之后的秘钥
-------------------------------------------------------*/
int getCipherRealLenByPriKey(int priKeyLen){
    ...
    if(priKeyLen == 319 || priKeyLen == 323)			/*256*/
        ...
        SignleRealLen = 128;								
    else if(priKeyLen == 1700 || priKeyLen == 1704 || priKeyLen == 1708)		/*2048*/
        SignleRealLen = 256;									
    else if(priKeyLen == 3268 || priKeyLen == 3272 || priKeyLen == 3276)		/*4096*/
        SignleRealLen = 512;									
